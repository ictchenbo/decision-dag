class LeafNode:
    ...
    def eval_node(self, data, errors: list):
        ...
        for operator in opera_set:
            if operator in self.expr:
                opera = operator
                left, right = self.expr.split(opera)[0].strip(), self.expr.split(opera)[1].strip()
...
def load(data: dict):  # 读取规则，并构造规则树
    node_list = []
